...
class SDPP(object):
    def __init__(self, config, sess, n_nodes):
        ...
        opt1 = tf.compat.v1.train.AdamOptimizer(learning_rate=self.learning_rate)
        opt2 = tf.compat.v1.train.AdamOptimizer(learning_rate=self.emb_learning_rate)
    ...
    def build_input(self):
        self.x = tf.compat.v1.placeholder(tf.int32, shape=[None, self.n_steps], name="x")
        # (total_number of sequence,n_steps)
        self.x_indict = tf.compat.v1.placeholder(tf.int64,shape=[None,3])
        # (total number of sequence,dim_index)
        self.y = tf.compat.v1.placeholder(tf.float32, [None, 1], name="y")
        self.time_interval_index = tf.compat.v1.placeholder(tf.float32, [None,self.n_time_interval], name="time")
        # (total_number of sequence,n_time_interval)
        self.rnn_index = tf.compat.v1.placeholder(tf.float32, [None,self.n_steps], name="rnn_index")
        # (total_number of sequence,n_steps)
        # self.rnn_length = tf.compat.v1.placeholder(tf.float32, [None,1], name="rnn_length")
    def build_var(self):
        with tf.variable_scope(self.name) as scope:
            with tf.variable_scope('embedding'):
                self.embedding = tf.compat.v1.get_variable('embedding', initializer= self.initializer2([self.n_nodes,self.embedding_size]),dtype = tf.float32)
            with tf.variable_scope('BiGRU'):
                self.gru_fw_cell = rnn.GRUCell(2*self.n_hidden_gru)
            with tf.variable_scope('SumPooling'):
                self.time_weight = tf.compat.v1.get_variable('time_weight', initializer=self.initializer([self.n_time_interval]), dtype=tf.float32)
		#self.time_weight = tf.mul(self.time_weight_temp,self.time_weight_temp)
            with tf.variable_scope('dense'):
                self.weights = {
                    'dense1': tf.compat.v1.get_variable('dense1_weight', initializer=self.initializer([2 * self.n_hidden_gru,
                                                                                        self.n_hidden_dense1])),
                   'dense2': tf.compat.v1.get_variable('dense2_weight', initializer=self.initializer([self.n_hidden_dense1,
                                                                                       self.n_hidden_dense2])),
                    'out': tf.compat.v1.get_variable('out_weight', initializer=self.initializer([self.n_hidden_dense2, 1]))
                }
                self.biases = {
                    'dense1': tf.compat.v1.get_variable('dense1_bias', initializer=self.initializer([self.n_hidden_dense1])),
                   'dense2': tf.compat.v1.get_variable('dense2_bias', initializer=self.initializer([self.n_hidden_dense2])),
                    'out': tf.compat.v1.get_variable('out_bias', initializer=self.initializer([1]))
                }
    ...
    def build_model(self):
        with tf.variable_scope('deephawkes') as scope:
            ...
            with tf.variable_scope('RNN'):
                ...
                outputs,states = tf.contrib.rnn.static_rnn(self.gru_fw_cell, x_vector, dtype=tf.float32)
    ...
    def get_embedding(self, x, x_indict,y, time_interval_index,rnn_index):
        embedding = self.sess.run([self.embedding],
                                                          feed_dict={self.x: x, self.x_indict: x_indict, self.y: y,
                                                                     self.time_interval_index:time_interval_index,
                                                                    self.rnn_index:rnn_index})
        # print rnn_state # feed_dict 替换原来中的输入
        return embedding
